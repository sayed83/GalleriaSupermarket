...
(function () {
    this.itemTransfer = this.itemTransfer || {};
    var ns = this.itemTransfer;

    ns.init = function () {
        var trans = new ns.Cart();
        trans.GetItemsForSale();
        trans.loadCart();
        trans.calculateNetAmount();
        ...
        $("#invTable").on("change", ".saleQnty", function () {
            ...
            if (newQnty > 0 && newQnty <= ckQnty) {
                trans.calculateQntyFromInput(name, size, newQnty);
                trans.calculateNetAmount();
            } else {
                if (newQnty <= 0) {
                    newQnty = 1;
                ...
                trans.calculateQntyFromInput(name, newQnty);
                trans.calculateNetAmount();
        ...
        $(document).on('click', '.addcart', trans.bindItemToCart);
        $('#transSubmit').on('click', trans.submitOrder);
        $('#showSuccess').hide();
        $('#showError').hide();
        $(document).on('click', '.deleteBtn', trans.deleteItemFromBox);
        $(document).on('click', '.subBtn', trans.cartItemOrderRemove);
        $(document).on('click', '.addBtn', trans.cartItemOrder);
        $(document).on('change', '.SizeList', trans.SizeWisePriceCalculation);
    };
    ...
    ns.Cart = (function () {
        ...
        function saveCart() {
            localStorage.setItem("itemTransfer", JSON.stringify(cart));
        };
        //Load cart Funciton
        function loadCart() {
            cart = JSON.parse(localStorage.getItem("itemTransfer"));
        };
        loadCart();
        ...
        Cart.prototype.submitOrder = function () {
            ...
            $("tr.item-row").each(function () {
                ...
                Products.push({ ItemID: itemid, PrName: name, AvailableQnty: qnty, ItemSize: size });
            });
            if (Outlet == 0) {
                $('#showError').show();
            } else {
                $.ajax({
                    url: '/ItemTransfer/Create',
                    type: 'POST',
                    traditional: true,
                    data: { transItem: JSON.stringify(Products), OutletId: Outlet},
                    dataType: 'json',
